...
class PPO:
	...
	"""
	def __init__(self, policy_class, env, is_adv, is_rarl, **hyperparameters):
		"""
			...
			Parameters:
				...
				is_adv - whether the agent is the adversary. 
			Returns:
				None
		"""
		...
		self.is_adv = is_adv
		self.is_rarl = is_rarl
		self.runner = None

		# Extract environment information depending on adversary or not
		self.env = env
		self.obs_dim = env.observation_space.shape[0]
		
		if(is_adv):
			self.act_dim = env.adv_action_space.shape[0]
		else:
	...
	def rollout(self): 
		...
		batch_obs, batch_acts, batch_log_probs, batch_rews, batch_lens = self.runner.rollout(self.timesteps_per_batch,self.is_adv)
	...
	def save(self, filename):
		...
		torch.save(self.critic_optim.state_dict(), filename + "_critic_optimizer.pt")
		torch.save(self.actor.state_dict(), filename + "_actor.pt")
		torch.save(self.actor_optim.state_dict(), filename + "_actor_optimizer.pt")
    
	def load(self, filename):
		self.critic.load_state_dict(torch.load(filename + "_critic.pt"))
		self.critic_optim.load_state_dict(torch.load(filename + "_critic_optimizer.pt"))
		self.actor.load_state_dict(torch.load(filename + "_actor.pt"))
		self.actor_optim.load_state_dict(torch.load(filename + "_actor_optimizer.pt"))
